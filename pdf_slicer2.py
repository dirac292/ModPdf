...
def analyze_pdf(file_path):
    ...
    for section, page_range in sections.items():
        pages.append(page_range[0])
        # print(f"Section {section}: Pages {page_range[0]} to {page_range[1]}")
...
def split(arr,output_path,input_pdf):
...
def main():
    ...
    #1 - 3 4 - 5 6 -7
    sections,pages = analyze_pdf(pdf_path)
    print(pages)
    arr = []
    # t = 0
    for i in pages:
    ...
    split(arr,output_path,input_pdf)
